...
def read_images():
    ...
    image_files = [file_name for file_name in os.listdir(input_directory) if file_name.lower().endswith(image_extensions)]

    # Loop through all image files with a progress bar
    for file_name in tqdm(image_files, desc="Processing images", unit="image"):
        if file_name.lower().endswith(image_extensions):  # Check if it's an image file
            ...
            # (Optional) Apply thresholding to preprocess the image
